...
#include "ui_zbsacontroldialog.h"
...
std::list<std::string> ZBSAControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZBSAControlDialog::listMints;

ZBSAControlDialog::ZBSAControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZBSAControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZBSAControlDialog::~ZBSAControlDialog()
{
    delete ui;
}

void ZBSAControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZBSAControlDialog::updateList()
{
...
void ZBSAControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZBSAControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZBSA_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZBSAControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZBSAControlDialog::GetSelectedMints()
{
...
void ZBSAControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
